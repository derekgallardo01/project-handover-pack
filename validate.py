"""Placeholder linter for the handover pack templates.

    python validate.py                       # check every .md under examples/
    python validate.py path/to/handover.md   # check one file (use this on your drafts)

Flags angle-bracket placeholders like `<client>`, `<date>`, `<flow name>` that
you forgot to fill in. Exit code 0 if clean, 1 if any remain — suitable for a
pre-deliver gate so half-finished handovers don't reach a client.
"""
